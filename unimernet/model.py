...
from components.tokenizer import DonutTokenizer
from components.encoder_decoder import UnimerNetEncoderDecoder
...
class UniMERModel:
    ...
    def __init__(
        ...
        self.model = UnimerNetEncoderDecoder(
    ...
    def _get_count_gt(self, text, device):
        labels = self.tokenizer.tokenize(text, max_length=1536)[
            "input_ids"].to(device)
        mask = labels != self.tokenizer.pad_token_id
        one_hot_labels = F.one_hot(
            labels, num_classes=self.tokenizer.tokenizer.vocab_size) * mask.unsqueeze(-1)
